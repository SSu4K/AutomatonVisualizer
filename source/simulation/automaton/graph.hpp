...
public:
    MooreLattice2D(size_vector size);
...
Node<T> &MooreLattice2D<T>::at(const int x, const int y){
    ...
    return this->nodes[wrap(x, width) + width*wrap(y, height)];
}
...
void MooreLattice2D<T>::create_neighbours(int x, int y){
    ...
    while(it<end){
        int dx = (*it)[0];
        int dy = (*it)[1];
        size_t index = wrap(x+dx, width) + width*wrap(y+dy, height);
...
template <class T>
MooreLattice2D<T>::MooreLattice2D(size_vector size){
    this->size = size;
    assert(size.size()==2);
