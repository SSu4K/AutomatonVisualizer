...
static Gradient g1({sf::Color::Yellow, sf::Color::Red,
...
vector<sf::Color> SimulationWindow::get_color_array(){
  ...
  for(size_t i=0; i<simulation->get_cell_count(); i++){
    sf::Color color = get_color(simulation->value_buffer[i], simulation->loaded_buffer[i],
                  simulation->age_buffer[i]);
    result.push_back(color);
    color.toInteger();
    //cout << hex << color.toInteger() << endl;
  }
  // std::cout << dec << result.size() << endl;

  return result;
}

void SimulationWindow::create_vertex_array(){
  vertex_array = sf::VertexArray(sf::PrimitiveType::Quads, 4*simulation->get_cell_count());
  ...
  // std::cout << vertex_array.getVertexCount() << endl;
}
...
SimulationWindow::SimulationWindow(sf::Vector2i window_size,
    ...
    : sf::RenderWindow(sf::VideoMode(window_size.x, window_size.y),
      ...
      framerate(rate),
      simulation_framerate(srate),
      frames_per_step(rate / srate),
      frame_counter(0),
      max_view_speed(100),
      drag_force(2000),
      show_grid(false),
  ...
  view = sf::View(center, sf::Vector2f(window_size)), view.zoom(std::max(w_ratio, h_ratio));
  setView(view);

  view_body =
      Body::Body(50, 10, Body::calculate_resistance(drag_force, max_view_speed),
                 0.01 * max_view_speed);
...
void SimulationWindow::step() {
  ...
  view_body.simulate(dt, drag_force * direction_vector);
